  ...
  <FormSection @submitted="createFolio">
    ...
    <template #form>
      <div class="col-span-6 sm:col-span-4">
        <InputLabel for="folio_type" value="What type of folio is being created?" />
        <select
          class="block w-full mt-1 border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm"
          v-model="form.folio_type"
          required
          name="folio_type"
          id="folio_type"
        >
          <option value="" disabled selected>Select Folio Type</option>
          <option value="accommodation">Accommodation</option>
          <option value="event">Event</option>
        </select>
        <InputError :message="form.errors.folio_type" class="mt-2" />
      </div>
      <template v-if="form.folio_type === 'accommodation'">
        <div class="col-span-6 sm:col-span-4">
