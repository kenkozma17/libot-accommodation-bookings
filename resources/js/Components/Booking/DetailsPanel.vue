...
function updateDates() {
    bookingDetails.dates.start = dayjs(bookingDetails.dates.start).format(
        "YYYY-MM-DD"
    );
    bookingDetails.dates.end = dayjs(bookingDetails.dates.end).format(
        "YYYY-MM-DD"
    );
    const daysDifference = dayjs(bookingDetails.dates.end).diff(
        dayjs(bookingDetails.dates.start),
        "day"
    );
    if (daysDifference > 0) {
...
onBeforeMount(() => {
    if (
        router.page.url === "/" ||
        router.page.url.includes("booking-details") ||
        router.page.url.includes("payment")
    )
...
<template>
    <div v-if="showPanel" id="details-panel">
        <div
            class="relative border-black border bg-black w-full grid grid-cols-4 gap-[1px]"
        >
            <div
                class="md:col-span-1 col-span-4 flex flex-col md:py-4 py-3 md:px-6 px-3 cursor-pointer bg-white"
            >
                <span class="font-bold text-[.75rem]">Hotel</span>
                <span class="text-[.75rem]">Catanduanes Midtown Inn</span>
            </div>
            <div
                @click="toggleGuestsAdjust"
                class="md:col-span-1 col-span-4 flex flex-col md:py-4 py-3 md:px-6 px-3 cursor-pointer relative bg-white"
            >
                <span class="font-bold text-[.75rem]">Guests</span>
                <span class="text-[.75rem]">{{
                    bookingStore.guestsCount
                }}</span>
            </div>
            <div
        ...
        <div class="w-full grid grid-cols-4 relative">
            <div
                v-if="showGuestsAdjust"
                class="md:col-start-2 md:col-end-3 absolute w-full z-50"
            >
            ...
            <div
                v-if="showDatePicker"
                class="md:col-start-3 w-full col-span-2 absolute z-50"
            >
